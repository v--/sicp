...
; change the currency used by the program, so that we could compute the number of ways to change
; a British pound, for example. As the program is written, the knowledge of the currency
...
; of the coins to use rather than an integer specifying which coins to use. We could then
...
; of primitive operations on list structures. Does the order of the list coin-values affect
...
;          └──cc(0, [2 4]) = 1
;
; The coin count equals the number of coin combinations for which the amount reduces to zero.
; Shuffling the coin list simply changes the order in which these "successful" call paths
; are evaluated. Since all other call paths evaluate to zero, the final result does not change.
